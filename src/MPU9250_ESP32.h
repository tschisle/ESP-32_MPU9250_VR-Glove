/*
 Notes: 
 This library is an adaptation of the MPU9250 library by Sparkfun to work with
 the ESP-32 microcontroller.
 The MPU9250 is an I2C sensor and uses the Arduino Wire library.
 The sensor is not 5V tolerant, the ESP-32 we're using is 3.3V.
 The sensor has an onboard linear voltage regulator so if any sensors run into 
 problems, it's advisable to solder the two surface pads below the regulator 
 together which is meant to bypass the regulator.  We have disabled the 
 internal pull-ups used by the Wire library in the Wire.h/twi.c utility file. 
...
class MPU9250
{
  protected:
    ...
    uint32_t _interfaceSpeed; // Stores the clock rate
